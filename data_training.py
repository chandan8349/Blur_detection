...
def extract_feature(dataset_dir):
...
def compile_train_data(feature_data_blur, feature_data_sharp, feature_data_motion_blur):
    ...
    train_data = np.concatenate((extracted_features_sharp, extracted_features_blur,
                                 extracted_features_motion_blur), axis=0, dtype='float64')
    return train_data
...
def balance_data(X_Train, Y_Train):
    ...
    # Separate the samples in positive and negative bin
...
if __name__ == '__main__':
    ...
    directory = './defocused_blurred/'
    feature_blur = extract_feature(directory)

    directory = './motion_blurred/'
    feature_motion_blur = extract_feature(directory)

    directory = './sharp/'
    feature_sharp = extract_feature(directory)

    train_data = compile_train_data(feature_blur, feature_sharp, feature_motion_blur)
